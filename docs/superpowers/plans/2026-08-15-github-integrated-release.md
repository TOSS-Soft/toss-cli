...
- Create `scripts/release-workflow-test.js`: static release-workflow contract test.
- Modify `.github/workflows/publish.yml`: validation, artifact, dual publication, release, and verification jobs.
- Modify `package.json`: append the release contract test to `npm test`.
...
- Produces: jobs `validate`, `publish_npm`, `publish_github_packages`, `release`; outputs `version`, `artifact_name`.
...
for (const token of [
  'merge-base --is-ancestor', 'npm test', 'npm pack',
  'actions/upload-artifact@v7', 'actions/download-artifact@v8',
  '@toss-software/cli', '@toss-soft/cli',
  'registry.npmjs.org', 'npm.pkg.github.com',
  'npm view', 'npm publish', 'github.token',
  'gh release create', '--verify-tag', '--generate-notes', '--latest',
  'gh release upload', '--clobber', 'gh release view',
...
assert.equal(JSON.parse(readFileSync(new URL('../package.json', import.meta.url))).version, '2.0.0');
...
Expected: FAIL with `missing job: validate` because the current workflow has only `publish`.

- [ ] **Step 4: Implement workflow header and validate job**
...
Use `actions/checkout@v7` with `fetch-depth: 0` and `actions/setup-node@v7` with Node 24. The `metadata` bash step must:
...
git merge-base --is-ancestor "$CHECKED_OUT_COMMIT" refs/remotes/origin/main
...
- [ ] **Step 5: Implement npmjs publication**
...
- [ ] **Step 6: Implement GitHub Packages publication**
...
NODE
npm pack ./github-package/package --ignore-scripts --pack-destination github-dist
...
- [ ] **Step 7: Implement gated GitHub Release creation**
...
- [ ] **Step 8: Verify GREEN and the complete payload**
...
```bash
git add .github/workflows/publish.yml scripts/release-workflow-test.js package.json
