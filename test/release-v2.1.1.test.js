...
test("release notes lock exact final release-candidate benchmark evidence",() => {
  const notes=readReleaseNotes();
  const evidence=[
  ...
  assert.ok(notes.includes(evidence),"release notes omit exact Task 6 evidence");
  const hashes=[...evidence.matchAll(/SHA-256\n?\s*`([0-9a-f]{64})`/g)]
    .map(match => match[1]);
