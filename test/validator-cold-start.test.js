...
test("the contract catalog is closed, complete, sorted, unique, and immutable",async () => {
  assert.equal(CONTRACT_SCHEMA_CATALOG.length,42);
  assert.deepEqual(
    CONTRACT_SCHEMA_CATALOG.map(row => row.schemaId),
    [...CONTRACT_SCHEMA_CATALOG.map(row => row.schemaId)].sort(),
  );
  assert.equal(new Set(CONTRACT_SCHEMA_CATALOG.map(row => row.schemaId)).size,42);
  assert.equal(new Set(CONTRACT_SCHEMA_CATALOG.map(row => row.uri)).size,42);
  assert.equal(new Set(CONTRACT_SCHEMA_CATALOG.map(row => row.relativePath)).size,42);
